...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "--benchmark") == 0 || strcmp(argv[1], "-b") == 0) {
            ...
            // Custom tile size
            g_tile_size = atoi(argv[1]);
            if (g_tile_size < 4 || g_tile_size > 256) {
                printf("Invalid tile size: %d. Using default TS=32\n", g_tile_size);
                g_tile_size = 32;
            } else {
                printf("Using custom tile size: TS=%d\n", g_tile_size);
            }
        }
    }
    ...
    // Benchmark mode - test multiple tile sizes
    if (benchmark_mode) {
        printf("===================== Benchmark Start ========================\n");
        benchmark_tile_sizes(images, network);
        printf("===================== Benchmark Complete =====================\n");
        // Clean up and exit (no need to run inference again)
        for (int i = 0; i < n; i++) free(probabilities[i]);
        free(probabilities);
        free_image_data(images);
        return 0;
    }
