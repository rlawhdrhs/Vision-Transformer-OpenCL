...
typedef struct {
    char name[64];       // 커널 이름 (예: "Linear", "Softmax")
    double total_time_ms; // 누적 실행 시간 (밀리초)
...
void run_linear(cl_mem in, cl_mem out, int w_idx, int b_idx, int tokens, int in_f, int out_f, int offset) {
    ...
    //// 2. WPT(4)로 나누어 글로벌 워크 아이템 개수 설정
    //// 예: 32x32 타일을 8x8 스레드가 처리하므로, 차원은 1/4로 줄어듬
...
void run_gemm(cl_mem A, cl_mem B, cl_mem C, int M, int N, int K, int transB,
    ...
    clSetKernelArg(g_opencl.k_gemm, 8, sizeof(int), &stride); // C_stride: MHA Score일 경우 다름 주의
...
void MHA_batched(cl_mem in, cl_mem out, int w_idx, int b_idx, int out_w_idx, int out_b_idx, int batch_size) {
    ...
    // Step 3: Softmax
    // -----------------------------------------------------
    // Global Size: [Batch * NumHeads, Tokens] (Score Kernel과 동일)
