...
// -------------------------------
// Corrected MLP_batched2
// -------------------------------
void MLP_batched2(
    cl_command_queue queue,
    ViT_Memory_Pool* pool,
    cl_mem in,
    cl_mem out,
    int w1_idx, int b1_idx,
    int w2_idx, int b2_idx,
    int batch_size)
{
    // ------- 변수 정리 -------
    const int TILE = 16;
    int tokens_per_img = ((img_size / patch_size) * (img_size / patch_size)) + 1;   // 197
    ...
    size_t l_fc[2] = { TILE, TILE };
    size_t g_fc1[2] = {
        ((size_t)total_tokens + TILE - 1) / TILE * TILE,
        ((size_t)hidden + TILE - 1) / TILE * TILE
    };
    size_t g_fc2[2] = {
        ((size_t)total_tokens + TILE - 1) / TILE * TILE,
    ...
    cl_mem d_fc1out = pool->mlp_fc1_out;  // <-- 전역 g_mem_pool 삭제
...
#ifdef ENABLE_PROFILING
    clEnqueueNDRangeKernel(queue, g_opencl.k_fc1_tiled, 2, NULL, g_fc1, l_fc, 0, NULL, &ev_fc1);
    register_kernel_time("FC1_tiled", ev_fc1);
    clReleaseEvent(ev_fc1);
#else
    clEnqueueNDRangeKernel(queue, g_opencl.k_fc1_tiled, 2, NULL, g_fc1, l_fc, 0, NULL, NULL);
#endif
...
#ifdef ENABLE_PROFILING
    clEnqueueNDRangeKernel(queue, g_opencl.k_gelu_tiled, 1, NULL, &g_gelu, &l_gelu, 0, NULL, &ev_gelu);
    register_kernel_time("GELU_tiled", ev_gelu);
    clReleaseEvent(ev_gelu);
#else
    clEnqueueNDRangeKernel(queue, g_opencl.k_gelu_tiled, 1, NULL, &g_gelu, &l_gelu, 0, NULL, NULL);
#endif
...
#ifdef ENABLE_PROFILING
    clEnqueueNDRangeKernel(queue, g_opencl.k_fc2_tiled, 2, NULL, g_fc2, l_fc, 0, NULL, &ev_fc2);
    register_kernel_time("FC2_tiled", ev_fc2);
    clReleaseEvent(ev_fc2);
#else
    clEnqueueNDRangeKernel(queue, g_opencl.k_fc2_tiled, 2, NULL, g_fc2, l_fc, 0, NULL, NULL);
#endif
}
